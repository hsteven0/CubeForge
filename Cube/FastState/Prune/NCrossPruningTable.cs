...
{
    public static class NCrossPruningTable
    {
        ...
        {
            target.GetSlotPieces(slot, out Corner targetCorner, out Edge targetEdge);
            ...
            PackCrossEdge(ref key, ref shift, state, targetEdge);
            PackCorner(ref key, ref shift, state, targetCorner);
        ...
        private static string GetTableKey(NCrossTarget target, F2LSlot slot, MoveSet moveSet)
        {
            ...
            return $"ncross_{SearchMoveSets.GetFileKey(moveSet)}_{slot}_{cross}_{(int)corner}_{(int)edge}";
        }

        private static void PackCrossEdge(ref ulong key, ref int shift, FastCubeState state, Edge edge)
        {
